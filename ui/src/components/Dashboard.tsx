...
function StatCard({
  ...
  return (
    <div className="bg-slate-800/60 border border-slate-700/60 rounded-xl px-4 py-3 flex flex-col gap-1">
...
function CostChart({ data }: { data: Metrics["cost_over_time"] }) {
  ...
  return (
    <div className="bg-slate-800/60 border border-slate-700/60 rounded-xl px-4 py-3">
      ...
      {data.length === 0 ? (
        <div className="text-slate-600 text-sm py-6 text-center">No runs yet</div>
      ) : (
            ...
            return (
              ...
              >
                <div className="flex-1 w-full flex items-end">
                  <div
                    className="w-full rounded-t bg-gradient-to-t from-indigo-600 to-indigo-400 group-hover:from-indigo-500 group-hover:to-indigo-300 transition-colors"
                    style={{ height: `${Math.max(pct, hasCost ? 2 : 0)}%`, minHeight: hasCost ? 2 : 0 }}
                  />
                </div>
                <div className="text-[9px] text-slate-600 tabular-nums truncate w-full text-center">
                  {d.date.slice(5)}
                </div>
              </div>
            );
...
function ModelTable({ models }: { models: Metrics["by_model"] }) {
  return (
    <div className="bg-slate-800/60 border border-slate-700/60 rounded-xl px-4 py-3">
      ...
      {models.length === 0 ? (
        <div className="text-slate-600 text-sm py-6 text-center">No LLM calls recorded</div>
      ) : (
        <table className="w-full text-sm">
          ...
          <tbody>
            {models.map((m) => (
              <tr key={m.model} className="border-t border-slate-700/50">
                <td className="py-1.5 text-slate-200 font-mono text-xs truncate max-w-[160px]" title={m.model}>
                  {m.model}
                </td>
                <td className="py-1.5 text-right text-slate-400 tabular-nums">{m.calls}</td>
                <td className="py-1.5 text-right text-slate-400 tabular-nums">
                  {fmtNum(m.input_tokens + m.output_tokens)}
                </td>
                <td className="py-1.5 text-right text-purple-300 tabular-nums font-medium">
                  {fmtCost(m.cost_usd)}
                </td>
              </tr>
            ))}
        ...
        {rows.map(({ kind, count }) => {
          ...
          return (
            <div key={kind} className="flex items-center gap-3">
              <span className="text-xs text-slate-400 w-12 capitalize">{kind}</span>
  ...
  return (
    <div className="flex-1 flex flex-col min-w-0">
      ...
      <div className="flex items-center gap-3 px-4 py-2 border-b border-slate-700 bg-slate-900 shrink-0">
        <span className="font-semibold text-slate-100">Analytics</span>
        {metrics && (
        ...
        <RefreshCw
          size={12}
          className={`ml-auto text-slate-600 ${loading ? "animate-spin" : ""}`}
        />
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-4">
        {!metrics ? (
          <div className="text-slate-500 text-sm text-center py-20">Loading metrics…</div>
        ) : metrics.run_count === 0 ? (
          <div className="flex flex-col items-center justify-center gap-3 py-24 text-center">
            <Activity size={26} className="text-indigo-400" />
            <p className="text-slate-400 text-sm">No runs to analyze yet.</p>
          </div>
        ) : (
          <div className="flex flex-col gap-4 max-w-5xl mx-auto">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
              <StatCard
                icon={<Layers size={12} />}
                label="Runs"
                value={`${metrics.run_count}`}
                sub={`${metrics.success_count} ok · ${metrics.error_count} err`}
              />
              <StatCard
                icon={<Gauge size={12} />}
                label="Success"
              ...
              <StatCard
                ...
                label="Avg duration"
                value={fmtDur(metrics.avg_duration_ms)}
              />
              <StatCard
                icon={<Cpu size={12} />}
                label="LLM calls"
