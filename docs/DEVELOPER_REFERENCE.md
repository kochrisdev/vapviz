...
- **Floor life / room doors (UI, Phase 4c)** — the floor stays alive between transitions. Each room gets a small theme-aware CSS door (`.vt-room-door`) on its Walk-Way-facing edge (the locked 12×16 diorama is untouched), and the walk overlay grows a persistent layer: `WalkEngine.reconcileFloorLife(busy, idle)` is called each poll from `BuildingView` (with a fresh `snapshot(el)` that now captures `roomdoor:<appKey>` anchors). It keeps one honest `Resident` (state machine `in → mill → out`, recolored by `appKey`) per **running** room, milling on that floor's Walk-Way lane and walking back through the door when the room finishes — plus an occasional decorative ambient stroller on any floor where nothing is running (honest and ambient motion never mix on a floor). Residents keep the rAF loop alive, so the engine is stopped explicitly on unmount; `prefers-reduced-motion` spawns none. UI-only (re-presents the derived graph), exempt from the dual-logic rule.
