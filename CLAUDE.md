...
.venv/bin/python -m pytest -m "not integration"              # fast/free Python unit tests (390 pass)
.venv/bin/python -m pytest                                   # everything incl. real-LLM integration (396 collected)
...
The data pipeline (the event lane, one direction):
...
- **Server (`vapviz/server.py`):** thin façade over the store. The SSE endpoint `GET /runs/{id}/events` is two-phase: replay history, then stream live from the queue (with periodic `ping`). Route-order gotcha: `/runs/compare` is declared before `/runs/{id}` so "compare" isn't parsed as a run id. **M3 (fixed 2026-07-18):** `create_app()` resolves `default_store` at *call* time, so `configure(db=...)` → `create_app()` works. The one remaining freeze point is the prebuilt `vapviz.app` (built at import) — it never follows a later `configure`; build your own app via `create_app()` instead.
...
- **The "events → graph" reduction is implemented twice and must stay in sync:** Python `_apply_event_to_graph` (`vapviz/store.py`) and TypeScript `applyEventToGraph` (`ui/src/store/runStore.ts`). **A cross-impl parity test guards this** (`tests/test_reducer_parity.py` ↔ `ui/src/store/runStore.parity.test.ts`, shared fixtures in `tests/fixtures/reducer_parity/`): both reducers replay the same events and must produce the same graph, so drift fails the gate. The event schema is also dual: `vapviz/events.py` (Pydantic) and `ui/src/types/events.ts` (TS). The per-run cost reduce is also dual (`_total_cost` ↔ `runStore.ts` — both LLM-kind only). **Adding/changing an event type, node kind, node status, or cost logic means editing all the relevant ones** — plus `KIND_BG` in `AgentGraph.tsx` and the `_start_event`/`_end_event` maps in `tracer.py`. See the Extension Guide in `docs/ARCHITECTURE.md`.
- **Control-channel boundary:** the `stopped` status and the inert `control` event ARE dual-logic (`_terminal_status` in `store.py` ↔ the runStore ternary — error > stopped > success — plus types and the `stopped_run`/`control_inert` parity fixtures). The live latch is NOT (`RunControl`/store control methods, `ui/src/lib/runControl.ts`, `RunControlBar.tsx` — side channel / UI-only).
- **`vapviz.configure(db=...)` ordering:** it swaps the module-level `default_store`. `trace`/`atrace` and `create_app()` both read `default_store` at call time, so for an in-process persistent server call `vapviz.configure(db=...)` **before** `vapviz.create_app()` and it just works (see `examples/crewai_demo.py`). Only the pre-built `vapviz.app` is frozen at import (M3 above) — don't pair it with `configure`.
...
- **`state_update` and `control` event types are inert** in both graph builders (they appear in the timeline but don't mutate the graph; `control` is the pause/resume/stop audit marker).
