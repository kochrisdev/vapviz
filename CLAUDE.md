...
.venv/bin/python -m pytest -m "not integration"              # fast/free Python unit tests (296 pass)
.venv/bin/python -m pytest                                   # everything incl. real-LLM integration (309 collected)
...
Personal (gitignored) notes live in `docs/notes/`: dated handoffs in **`docs/notes/handoffs/`** (`HANDOFF-YYYY-MM-DD.md`, one per session — **read the newest first in a new session**; latest is `HANDOFF-2026-06-25.md`), **SESSION-FINDINGS.md** (the bug work order with repro/fix/verify steps + FIXED stamps), **UI-ROADMAP.md** (the legible/watchable UI plan — Phase 3 Theater DONE; Phase 4 left), REFERENCE.md (plain-English explainer), NEXT-SESSION-STARTER.md. Throwaway test scripts belong in `scratch/` (gitignored; `scratch/.env` holds the OpenRouter key — never print or commit it).
...
**Theater / Live Floor (UI Phase 3, 2026-06-25):** `ui/src/lib/avatar.ts` (deterministic pixel-character SVG engine — bespoke, no DiceBear) · `ui/src/lib/theater.ts` (pure `buildScene` cast/state/desk reduction) · `ui/src/components/AgentStage.tsx` (the shared room + walking-avatar renderer) · `TheaterView.tsx` (per-run tab) · `FloorView.tsx` (global monitor, `view:"floor"`, polls `/runs`+`/graph`). **UI-only** — `lib/avatar.ts`/`lib/theater.ts`/`AgentStage` are *not* part of the dual-logic rule (they re-present the already-derived graph). The one backend touch is an **additive** `langgraph_node` marker in `vapviz/integrations/langchain.py` (rides into `node.data`; no event→graph change). Theater walking choreography is UI presentation; cost/graph-reduction logic is untouched.
