...
Personal (gitignored) notes live in `docs/notes/`: dated handoffs in **`docs/notes/handoffs/`** (`HANDOFF-YYYY-MM-DD.md`, one per session — **read the newest first in a new session**), **SESSION-FINDINGS.md** (the bug work order with repro/fix/verify steps + FIXED stamps), **UI-ROADMAP.md** (the legible/watchable UI plan — Phase 3 Theater DONE; Phase 4 left), REFERENCE.md (plain-English explainer), NEXT-SESSION-STARTER.md. Throwaway test scripts belong in `scratch/` (gitignored; `scratch/.env` holds the OpenRouter key — never print or commit it).
